...
class VSSMACoachEnv(VSSBaseEnv):
    ...
    def _frame_to_observations(self):
        ...
        manager = []
        manager.append(self.norm_pos(self.frame.ball.x))
        manager.append(self.norm_pos(self.frame.ball.y))
        manager.append(self.norm_v(self.frame.ball.v_x))
        manager.append(self.norm_v(self.frame.ball.v_y))
        manager += robots[0]
        for i in range(self.n_robots_yellow):
            manager.append(self.norm_pos(self.frame.robots_yellow[i].x))
            manager.append(self.norm_pos(self.frame.robots_yellow[i].y))
            manager.append(self.norm_v(self.frame.robots_yellow[i].v_x))
            manager.append(self.norm_v(self.frame.robots_yellow[i].v_y))
            manager.append(self.norm_w(self.frame.robots_yellow[i].v_theta))

        observations.append(np.array(manager, dtype=np.float32))
    ...
    def ball_stuck(self): 
        ...
        res = list()
        if len(too_close) > 1:
            res = [x[0] for x in closests[1:] if x[1] < min_dist]
        return res
